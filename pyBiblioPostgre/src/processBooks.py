...
sql = """INSERT INTO tbllibros(libro_id, titulo, autor_id, editorial_id, pags, isbn, ejemplares, leido, observaciones)
    VALUES(%(lid)s, %(titulo)s, %(aid)s, %(eid)s, %(pags)s, %(isbn)s, %(numl)s, %(readed)s, %(obs)s)
    RETURNING id;"""
...
def insert_all(data, new_id_col):
    ...
    for index in data.index:
        ...
        obs = str(data.loc[index, 'observaciones']).strip()
...
if __name__ == '__main__':
    ...
    save_file(data, filename='data/all_books.csv', cols=all_cols)
